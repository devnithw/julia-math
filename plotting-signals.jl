...
# ╔═╡ fe61ede2-21bf-40cd-a34f-ac8d0cb1c635
begin
	using Plots
	theme(:lime)
end

# ╔═╡ 23e260ce-bfe9-11ed-0fac-8deebfa480f9
md"""# Plotting common continuous time and discrete time signals in Julia

\

!!! warning "Introduction"
	In this tutorial use the `Plots.jl` package to plot some commonly encounteres signals patterns in both continuous and discrete time cases. 
"""

# ╔═╡ 7d9f67ce-71f9-44d1-87e6-98f0cc778cce
md"Let's import the plots package first and change the theme to lime for ✨ elegance ✨"

# ╔═╡ 58e690c5-4425-4214-9908-dba38e200abf
md"
## Sinusoidal signals

First let's plot some basic sinusoids with a phase shift.
"

# ╔═╡ a35a4639-2d9d-4bcd-8eeb-45c0d0b90917
md"
### Continous time sinusoid
"

# ╔═╡ 00bce1a1-dc42-4e65-a3a3-2f5a0b452548
begin
	# Parameters
	A = 1.0
	ϕ = pi / 4
...
# ╔═╡ 7dc51072-5440-4781-8bfa-2ca4582a2184
# Plot the sinusoid
plot(x, -10*pi, 10*pi, lw=1, framestyle = :origin, label=false)

# ╔═╡ 2b045348-035b-430a-89f2-368a187648af
md"

### Discrete time sinusoid"

# ╔═╡ 36064cb6-9e25-4c40-80bb-9fe985e64338
begin
	# Create an array of discrete values (integers)
	ns = [-30:1:30;]
	xs = x(ns)

	# Plot a stem graph
	plot(ns, xs, line=:stem, marker=:circle, markersize=3,framestyle = :origin, label=false)
end

# ╔═╡ 9fb5711b-a3d3-409b-946f-b40d2ad5c980
md"""
Observe the periodicity of this time signal. In the continuous time case it is obviously periodic.
!!! correct "Remember"
	All continuous time sinusoids are periodic 

But in the discrete case it is different the above discrete time signal is periodic, with a period of $N = 16$. Let's look at some different signals.

"""

# ╔═╡ acb5db94-8421-4407-882b-d5d580bfdcfa
begin
	# Assume zero phase shift for convenience
	# Set a new angular frequencies
	ω1 = 2 * pi / 12
	ω2 = 2 * pi / 6
	ω3 = 8 * pi / 31
	ω4 = 2 / 3
	...
	# Plot a stem graph of a different discrete time signal
	p1 = plot(n_array, x1 , line=:stem, marker=:circle, markersize=3,framestyle = :origin, title="ω1 = 2π/12", label=false)
	p2 = plot(n_array, x2 , line=:stem, marker=:circle, markersize=3,framestyle = :origin, title="ω2 = 2π/6", label=false)
	p3 = plot(n_array, x3 , line=:stem, marker=:circle, markersize=3,framestyle = :origin, title="ω3 = 8π/31", label=false)
	p4 = plot(n_array, x4 , line=:stem, marker=:circle, markersize=3,framestyle = :origin, title="ω4 = 2/3", label=false)

	# Set layout and resize
	plot(p1, p2, p3, p4, layout=(4,1), xticks=false, yticks=false)
	plot!(size=(800,700))
...
md"""
Notice how the first and the second sinusoid signals are clearly periodic. We can find a periodic pattern. The second one has twice the frequency of the first.

In the third graph, it's harder to spot the periodicity. But pick one bar and go 4 cycles to the right and you will find the next bar periodic with the starting one. *This signal becomes periodic after 4 periods of the continous time envelop*.

But in the fourth graph, you cannot find two points with periodicity. The signal is symmetric about the y-axis. This signal is aperiodic.

!!! correct "Remember"
	Discrete time sinusoids *can be* aperiodic. Particularly when π does not come up in the angular frequency term. We say it's period is ∞
...
>## References
>This tutorial notebook is based on an engineering module at University of Moratuwa called EN1020 Circuits, Signals and Systems. The course is taught by [Dr. Ranga Rodrigo] (https://uom.lk/staff/Rodrigo.BKRP) and I used the examples he discussed in class to make the plots.
