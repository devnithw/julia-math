...
# ╔═╡ a31966cc-4f53-439b-ad0c-d6524847a71c
# Since this is an ODE we only need to import the OrdinaryDiffEq module from DifferentialEquations package
using OrdinaryDiffEq, Plots

# ╔═╡ 4a7eac9c-a383-11ed-25bd-979f8f477850
md"""# Harmonic Oscillators and Solving ODEs

\

!!! warning "Introduction"
	In this tutorial we will have a look at **harminic oscillators** (mass on a spring) under two cases. We will obtain differential equations for the motion of such systems and use Julia's `DifferentialEquations` module to solve them. 
"""

# ╔═╡ 589786e7-e5be-4ce1-9471-90e623359dde
md"## What are Harmonic Oscillators?

In classical mechanics, a harmonic oscillator _is a system that, when displaced from its equilibrium position, experiences a restoring force F proportional to the displacement x_ ([wikipedia](https://en.wikipedia.org/wiki/Harmonic_oscillator))

Examples of such systems,
- Simple pendulum
- Mass fixed to a spring
- Bouy oscillating on a fluid surface

We will first look at how we can analyse the motion of such a system using Newton's laws of mechanics.
"

# ╔═╡ 0969d556-10e7-4b39-8e40-33651281f317
md"
## Simple Mass-Spring system

The below diagram shows a mass of $m$ fixed to a spring with a spring constant $k$   and obeys Hooke's Law. It is released from the equilibrium position with an initial velocity of 10m/s. It is on the _horizontal plane_ so we won't have to consider gravity. For this example we will ignore _damping effect_. Suppose the displacement of the center of mass is $x$ at time $t$.

[image here]

Based on Hooke's Law we can write the following equation for the spring. Here F is the restoring force when the spring is streched. Notice the displacement and force are in opposite directions, hence the negative sign.

$F_{restoring} = -kx$

And considering the motion of the mass we can write Newton's 2nd law as below.

$F_{restoring} = m\ddot{x}$

Now we will equate these two equations and obtain a _2nd order linear differential equation_.

$m\ddot{x} = -kx$

Substituting the values in the problem we have,

$50\ddot{x} = -200x$
$50\ddot{x} + 200x = 0$

This is a 2nd order homogeneous linear differential equation with initial conditions,

$x(0) = 0, \quad \dot{x}(0) = 10$


"

# ╔═╡ 9e4dd537-f6c3-490a-9aa7-d3a8f0805569
md"
## ODE solution with Julia

Since this is a easy ODE we can solve this by hand by using the substitution,

$x = e^{\lambda t}$

and obtaining the characteristic equation,

$50\lambda ^{2}+200\lambda = 0$

But in this tutorial we will straight up use Julia to solve this.
Our ODE simplified is,

$\ddot{x} + 4x = 0$

Observe that the natural frequency $w$ of this harmonic system is $2$. Since,

$\ddot{x} + {\omega}^2x = 0 => {\omega}^2 = 4$

"

# ╔═╡ d18d4c58-9622-451a-860e-1c895bba95f9
md"
First import Julia's `DifferentialEquations` package. We will also import Plots to plot our results
"

# ╔═╡ 68264c97-d1ce-4cd0-8654-4679e92117a1
begin
	# Parameters
	ω = 2
...
# ╔═╡ c116cfa5-576a-4f95-80bc-b939303b84bb
md"Now let's define the ODE problem in a function"

# ╔═╡ e6ebab8a-cc15-420e-8577-250fc55f2b6b
#Define the ODE structure
function harmonicoscillator(ddu,du,u,ω,t)
	ddu .= -ω^2 * u
end

# ╔═╡ 38c4eb74-c2d7-4230-8c1a-bd95e0adab7d
md"Then use the module's ODE solving algorithm to solve this system and get a vector output. Notice how we pass in the function, initial conditions and parameters."

# ╔═╡ 145a5406-fa87-4bbc-8cf7-3d3506f597bb
begin
	#Pass to solvers
	prob = SecondOrderODEProblem(harmonicoscillator, ẋ₀, x₀, tspan, ω);
	sol = solve(prob, DPRKN6());
end

# ╔═╡ b7272bb9-76ea-4966-b581-cc84b0d6e4cd
md"Let's plot this in a graph and look at what we got.

I have also solved the equation by-hand and obtained the analytical solution to be,

$x(t) = 5sin(2t)$

Let's plot that also to make sure our calculation is correct.
"

# ╔═╡ a7deef40-e252-4fc3-b9bf-a7b8ef91df28
# ╠═╡ show_logs = false
begin
	# Numerical solution
	plot(sol, vars=[2,1], linewidth=2, title ="Mass-Spring system (without damping)", xaxis = "time", yaxis = "distance/velocity", label = ["displacement" "velocity"])
	
	# Analytical solution
	plot!(t->5*sin(2*t), lw=3, ls=:dash, label="Analytical Solution displacement")
end

# ╔═╡ 7f5f9f4a-8432-47aa-8306-409ba8b11221
md"And indeed Julia's numerical solution output is congruent with the analytical solution.

Now we will add damping to this system and observe the motion."

# ╔═╡ d1a2128f-7b99-4c90-980c-fa46a18d8222
md"## Mass-Spring system with damping
Suppose we add a damper to this system with a damping ratio of $d=0.2$.

[image here]

Then our equation of motion translates into,

$\ddot{x} +2{\omega}d\dot{x} +{\omega}^2x=0$

For more details on damping and equation of motion, read [wikipedia](https://en.wikipedia.org/wiki/Damping). Substituting values we get,

$\ddot{x} +0.8\dot{x} +4x=0$

Let's plug this into the ODE solver to get a numerical solution.
"

# ╔═╡ f4ee7d66-df82-4aae-b007-411874ecbbb7
# Define paramaters
p = [2, 0.2] # ω and d

# ╔═╡ 32885564-8c8d-414a-a725-42f6a502fba7
#Define new ODE structure
function dampedharmonicoscillator(ddu,du,u,p,t)
	ddu .= -p[1]^2 * u -2*p[2]*p[1]*du
end

# ╔═╡ 69ecc545-2f31-4064-8d69-a0d3f5d0cb2b
begin
	#Pass to solvers
	prob2 = SecondOrderODEProblem(dampedharmonicoscillator, ẋ₀, x₀, (0.0, 4π), p);
	sol2 = solve(prob2, DPRKN6());
end

# ╔═╡ 964f8e1b-cf0e-4d92-8d3e-602fbeb26270
# ╠═╡ show_logs = false
begin
	# Numerical solution
	plot(sol2, vars=[2,1], linewidth=2, title ="Mass-Spring system (with damping)", xaxis = "time", yaxis = "distance/velocity", label = ["displacement" "velocity"])
end

# ╔═╡ dccf1068-6166-486c-87d5-a67291347f41
md"We can observe how the damping effect takes place. Play around with different values of ω and d to see various behavior in the graph."

# ╔═╡ 6e16c755-0e62-4022-a4fc-1a5280de0376
md"## Conclusion
In this tutorial we,
- Saw what a harmonic oscillator is and examples
- Modelled a simple mass-spring system  using classical mechanics and obtained an ODE
- Learned how to solve such an ODE using Julia's `DifferentialEquations` package
- Added damping to the system and observed the behavior
...
>## References
>This tutorial notebook is based on a video lecture by Steven L. Brunton [Example Second-Order ODE: Spring-Mass-Damper](https://youtu.be/r1eWerqrcqo)
>
>The example in this tutorial is taken from [Advanced Engineering Mathematics By Dennis G. Zill](https://books.google.lk/books?id=tFARDQAAQBAJ) (Spring-Mass systems)
>
> For more details on solving ODEs using Julia, check out [DifferentialEquations.jl](https://docs.sciml.ai/DiffEqDocs/stable/examples/classical_physics/)

\

"

# ╔═╡ c111a93f-199a-4f9c-9743-d6e946e80997
md"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "765e57332f803d538797dfae087424b18c47869b"
